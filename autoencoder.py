...
class ResNet_autoencoder(nn.Module):
    def __init__(self, downblock, upblock, 
                num_layers, n_classes,
                enc_out_dim=2048, latent_dim=8192):
        ...
        self.fc_rev = nn.Linear(latent_dim, enc_out_dim)
    ...
    def decoder(self, x, image_size):
        ...
        print("Last decoder var X shape: ", x.shape)
    ...
    def forward(self, x):
        ...
        enc_out = torch.transpose(enc_out, 1, 3)
        tmp1 = self.fc(enc_out)
        tmp1to2 = self.fc_rev(tmp1)
        tmp1to2 = torch.transpose(tmp1to2, 3, 1)
        # tmp1 = enc_out
        tmp2 = self.decoder(tmp1to2, x.size())
        return tmp1, tmp2
